...
const std::string keyvalues::GetString() const
{
	if (UsesEscapeSequence) {
...
void keyvalues::PushUserData(const std::string& name, void* data)
{
	userdata[name] = data;
}

void* keyvalues::GetUserData(const std::string& name)
{
	for (auto& map : userdata)
	{
		if (map.first == name)
		{
			return map.second;
...
void keyvalues::DeleteUserData(const std::string& name)
{
...
bool keyvalues::IsUserData(const std::string& name)
{
	return (userdata.find(name) != userdata.end());
}